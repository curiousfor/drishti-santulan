...
const MoodInput = ({ id, label, value, onChange, min = 1, max = 5, lowLabel, highLabel }) => {
    
    // 1. Define emoji sets for each category
    const emojiMap = {
        mood:    ['😫', '😕', '😐', '🙂', '🤩'], // Sad → Happy
        stress:  ['🤯', '😓', '😐', '🙂', '😌'], // Relaxed → Overwhelmed
        anxiety: ['😨', '😬', '😐', '🙂', '😌'], // Calm → Nervous
        energy:  ['🪫', '🥱', '😐', '🔋', '⚡'], // Drained → Charged
        sleep:   ['🧟', '🥱', '😐', '😌', '🤩'], // Poor → Great
        focus:   ['😵‍💫', '😶', '🤔', '🧐', '🎯'], // Distracted → Focused
        social:  ['😶', '😕', '😐', '🤝', '🥳'], // Lonely → Connected
        optimism:['🌧️', '☁️', '⛅', '☀️', '✨'], // Pessimistic → Hopeful
        default: ['😫', '😕', '😐', '🙂', '🤩']
    };

    // 2. Pick the right set (or use default if ID is missing)
    const currentEmojis = emojiMap[id] || emojiMap.default;

    const getEmoji = (val) => {
        if (val <= 1.5) return currentEmojis[0];
        if (val <= 2.5) return currentEmojis[1];
        if (val <= 3.5) return currentEmojis[2];
        if (val <= 4.5) return currentEmojis[3];
        return currentEmojis[4];
    };
    ...
    return (
        <div className="space-y-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:border-indigo-300 transition-all">
            ...
            {/*This is changed so to align the values to center */}
            <div className="relative flex justify-between items-center text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                <span className="relative z-10">{lowLabel}</span>
                
                {/* Added absolute positioning to force exact center */}
                <span className="absolute left-1/2 -translate-x-1/2 text-indigo-600 font-bold bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">
                    {Number(value).toFixed(1)}
                </span>
                
                <span className="relative z-10">{highLabel}</span>
            </div>
        </div>
    );
};
...
const StudentDashboard = ({ recoveryKey, setView, studentHistory, onLogSubmit, syncStatus, onSync }) => {
  ...
  const questions = [
    { key: 'mood', label: 'Overall Mood', low: 'Sad', high: 'Happy', icon: Smile },
    { key: 'stress', label: 'Stress Level', low: 'Burnt Out', high: 'Manageable', icon: CloudRain },
    ...
    { key: 'anxiety', label: 'Anxiety', low: 'Nervous', high: 'Calm', icon: Wind },
    { key: 'focus', label: 'Focus', low: 'Foggy', high: 'Clear', icon: Activity },
    { key: 'social', label: 'Social Connection', low: 'Lonely', high: 'Connected', icon: Users },
    { key: 'optimism', label: 'Outlook', low: 'Gloomy', high: 'Bright', icon: Sun },
  ];
  ...
  return (
    ...
    <div key={q.key} className={`transform transition-all hover:scale-[1.01]`}>
        <MoodInput 
            id={q.key}           // <--- i added this line so it can understand which emoji tou use for what indicator 
            label={q.label} 
            value={form[q.key]} 
